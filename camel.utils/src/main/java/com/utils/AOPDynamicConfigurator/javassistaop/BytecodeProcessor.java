...
public class BytecodeProcessor implements IBytecodeProcessor {
    ...
    public static BytecodeProcessor getBytecodeProcessor(String supported) {
        ...
        }else if (supported.equalsIgnoreCase(AOPSupportEnum.Log4j.name())) {
            return new Log4jPlaceHolderProcessor();
        } else if(supported.equalsIgnoreCase(AOPSupportEnum.Mybatis.name())){
            return new MybatisPlaceHolderProcessor();
        } else {
            return null;
        }
    }
